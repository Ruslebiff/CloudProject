...
func HandlerMeal(w http.ResponseWriter, r *http.Request) {
	...
	for _, list := range recipeList { //Goes through all recipes
		...
		case http.MethodPost:
			{
				err := json.NewDecoder(r.Body).Decode(&recipeTemp.Ingredients.Remaining)
				if err != nil {
		...
		case http.MethodGet:
			{
				recipeTemp.Ingredients.Remaining = ReadIngredients(strings.Split(r.URL.Query().Get("ingredients"), "_"))
			}
		}
